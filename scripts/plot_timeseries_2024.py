...
def get_model_predictions_temporal(df, stations):
    ...
    # Temporal split using EXACT year_slices from builder
...
def plot_station(stn_name, dates, obs, model_pred, ecmwf_pred, gfs_pred,
    ...
    v_o = ~np.isnan(o)
    ax.plot(d[v_o], o[v_o], color=obs_c, linewidth=1.4, alpha=0.9,
            marker="o", markersize=5, markerfacecolor=obs_c,
    ...
    v_m = ~np.isnan(m)
    if v_m.sum() > 0:
        ax.plot(d[v_m], m[v_m], color=mod_c, linewidth=1.3, alpha=0.85,
                marker="*", markersize=7, markerfacecolor=mod_c,
    ...
    v_e = ~np.isnan(e)
    if v_e.sum() > 0:
        ax.plot(d[v_e], e[v_e], color=ecmwf_c, linewidth=1.2, alpha=0.75,
                marker="^", markersize=4.5, markerfacecolor=ecmwf_c,
    ...
    v_g = ~np.isnan(g)
    if v_g.sum() > 0:
        ax.plot(d[v_g], g[v_g], color=gfs_c, linewidth=1.2, alpha=0.75,
                marker="s", markersize=4, markerfacecolor=gfs_c,
                markeredgecolor="white", markeredgewidth=0.4,
    ...
    ax.legend(loc="upper right", fontsize=12, frameon=True, fancybox=False,
              ...
              prop={"weight": "bold", "size": 12}, ncol=1)
...
def main():
    ...
    for _, srow in stations.iterrows():
        ...
        out_path = OUT_DIR / f"timeseries_2024_{stn_name}.png"
