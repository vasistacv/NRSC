...
class RainfallDataBuilder:
    ...
    def __init__(self, window_size: int = 3, station_filter: str = None):
        ...
        # Filter to a single station if requested
        if self.station_filter is not None:
            self.gt = self.gt[self.gt["Station"] == self.station_filter]
            print(f"  -> Filtered to station: {self.station_filter}")

        print(f"  -> {len(self.gt):,} valid ground-truth rows after filtering")
...
def get_dataloaders(window_size: int = 3, station_filter: str = None):
    builder = RainfallDataBuilder(window_size=window_size, station_filter=station_filter)
